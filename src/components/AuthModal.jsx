  ...
  useEffect(() => {
    ...
    if (!verifierRef.current) {
      verifierRef.current = new RecaptchaVerifier(
        auth,                          // ✅ THIS WAS MISSING — auth must be first arg
        recaptchaContainerRef.current, // container second
        { size: "invisible" }
      );
    }

    return () => {
      // ✅ Properly clear the widget so it can be re-rendered on next open
      if (verifierRef.current) {
    ...
    <div className="auth-modal-overlay" role="dialog" aria-modal="true">
      <div className="auth-modal" onClick={(e) => e.stopPropagation()}>
        <div className="auth-modal-header">
          <div className="auth-modal-title">{mode === "login" ? "Login" : "Signup"}</div>
        ...
        {step === "phone" ? (
          <form className="auth-form" onSubmit={handleSendOtp}>
            ...
            <button className="primary" type="submit" disabled={!canSendOtp || loading}>
          ...
          <form className="auth-form" onSubmit={handleVerify}>
            ...
            <button className="primary" type="submit" disabled={loading || otp.trim().length < 4}>
        ...
        <div className="auth-footer-note">By continuing, you agree to the Terms & Privacy Policy.</div>
      </div>
    </div>
  );
}